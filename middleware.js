...
function requiresAuth(url, method) {
  ...
  const publicRoutes = ['/login', '/register', '/confirm-email', '/resend-code', 'recovery-verify', '/logs', 'reset-password', 'verify-reset-link', 'change-password', 'plans', 'verify-mfa', '/api/swagger', '/request-magic-link', '/magic-link', 'accept-invitation', 'recover-account', 'download', '/webauthn/complete-authenticate'];
  let isverify = false;
